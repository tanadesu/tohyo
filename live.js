...
function render(){
  const keys=Number(event.optionCount)===2?['a','b']:['a','b','c'],names={a:event.nameA,b:event.nameB,c:event.nameC},total=keys.reduce((sum,key)=>sum+votes[key],0),max=Math.max(...keys.map(key=>votes[key])),rankedKeys=[...keys].sort((left,right)=>votes[right]-votes[left]||keys.indexOf(left)-keys.indexOf(right));
  $('#round').textContent=`ROUND ${Number(event.round)||1} / LIVE`;$('#liveQuestion').textContent=event.question;
  ...
  const root=$('#liveChoices');root.innerHTML='';keys.forEach(key=>{const count=votes[key],percent=total?Math.round(count/total*100):0,leading=total>0&&count===max,rank=rankedKeys.indexOf(key)+1;const card=document.createElement('article');card.className=`screen-choice choice-${key} rank-${rank}${leading?' winner':''}`;card.innerHTML=`<div class="choice-top"><span>SIDE ${key.toUpperCase()}</span>${leading?'<b>LEADING</b>':''}</div><h2></h2><div class="choice-score"><strong>${percent}%</strong><span>${count.toLocaleString('ja-JP')}票</span></div><div class="choice-meter"><i style="--score:${percent}%"></i></div>`;card.querySelector('h2').textContent=names[key]||`候補 ${key.toUpperCase()}`;root.appendChild(card)});
}
