...
function render(item){
  ...
  const names={a:item.nameA,b:item.nameB,c:item.nameC},total=Number(item.total)||keys.reduce((sum,key)=>sum+(item.votes?.[key]||0),0);
  const max=Math.max(...keys.map(key=>item.votes?.[key]||0));
  $('#round').textContent=`ROUND ${item.round} / FINAL RESULT`;$('#resultQuestion').textContent=item.question;$('#screenTotal').textContent=total.toLocaleString('ja-JP');
  const root=$('#screenChoices');root.innerHTML='';
  keys.forEach((key,index)=>{const count=item.votes?.[key]||0,percent=total?Math.round(count/total*100):0,winner=total>0&&count===max;const card=document.createElement('article');card.className=`screen-choice choice-${key}${winner?' winner':''}`;card.style.setProperty('--delay',`${index*120}ms`);card.innerHTML=`<div class="choice-top"><span>SIDE ${key.toUpperCase()}</span>${winner?'<b>WINNER</b>':''}</div><h2></h2><div class="choice-score"><strong>${percent}%</strong><span>${count.toLocaleString('ja-JP')}票</span></div><div class="choice-meter"><i style="--score:${percent}%"></i></div>`;card.querySelector('h2').textContent=names[key]||`候補 ${key.toUpperCase()}`;root.appendChild(card)});
